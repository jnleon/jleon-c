...
namespace Draw.Tool
{
	...
	class WoodenPencil
	{
		...
		///
		/// Note that Length and Sharpnes are 'private set' since they are adjusted
		///   over time as the pencil writes and is sharpened,
		///   but shouldn't be settable from outside class.
		///
		/// They rest of the propties are set only once in the constructor, and have
		///   no set.
		/// </summary>
		...
		public WoodenPencil(double Length)
        {
			this.Length = Length; 
        }
		public WoodenPencil(double length, int shape, string hardness, Color color, double sharpness)
		{
			...
			{
				Length = length;
				//length = this.Length; this does NOT do the same thing. it sets the parameter to instance variable
			}
			else
			{
        ...
        {
			return string.Format("Length: {0} Shape: {1} Hardness: {2} Sharpness: {3} IsStub: {4}", Length, Shape, this.Hardness, this.Sharpness, this.IsStub);
        }

		public void OverLoad()
        {
			Console.WriteLine("you have called method named OverLoad with no parameters");
        }

		public void OverLoad(int number)
		{
			Console.WriteLine("you have called method named OverLoad with one int parameter");
		}

		public void OverLoad(string str)
		{
			Console.WriteLine("you have called method named OverLoad with one string parameter");
		}
