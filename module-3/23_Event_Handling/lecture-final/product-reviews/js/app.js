...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  //add a listener so if the user clicks on description, we call toggleDescriptionEdit
  const desc = document.querySelector('.description');
  desc.addEventListener('click',(event)=>{
    toggleDescriptionEdit(event.target);
  })

  //add a listener to the textbox to call exitDescriptionEdit if they hit escape or enter
  ...
  //add an event listener to the textbox so if the mouse leaves then it does the same as teh esc key
  inputDesc.addEventListener('mouseleave',(event)=>{
    exitDescriptionEdit(event.target,false);
  })

  //add an evetn listener to the btnToggleForm button to call showHideForm
  const btnToggleForm = document.getElementById('btnToggleForm');
  btnToggleForm.addEventListener('click',showHideForm)

  //add an event listener to the save review button to prevent the refresh and save the review
...
function showHideForm() {
  ...
  if (form.classList.contains('d-none')) { //it's hidden rn
    form.classList.remove('d-none');
    btn.innerText = 'Hide Form';
    document.getElementById('name').focus();
  } else {//we're already showing the form
    resetFormValues();
...
function saveReview() {
  ...
  //create a new review object based on the fields that the user filled in
  const newReview = {
    reviewer: name.value,
    title: title.value,
    review: review.value,
  ...
  //then we want to add that to the list of reviews
  reviews.push(newReview);
  //call displayReview for my new review
