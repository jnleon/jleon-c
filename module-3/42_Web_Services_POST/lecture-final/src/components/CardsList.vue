...
export default {
  ...
  methods: {
    ...
    deleteBoard() {
      if (
        confirm(
          "Are you sure that you want to delete this board and all cards? There is no undo."
        )
      ) {
        boardsService
          ...
          .catch((error) => {
            if (error.response) {
              this.errorMsg =
                "Error deleting board. Response from server was " +
                error.response.statusText +
                ".";
            } else if (error.request) {
              this.errorMsg = "Error deleting board. Could not reach server.";
            } else {
              this.errorMsg =
                "Error deleting new board. Request could not be created.";
            }
          });
      }
    },
